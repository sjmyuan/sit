...
export const putObject =
  (s3: S3, bucket: string, cdn: O.Option<string>) =>
  (key: string, blob: Blob): AppErrorOr<S3ObjectInfo> => {
    return pipe(
      TE.tryCatch(
        () =>
          s3
...
export const putObjects =
  (s3: S3, bucket: string, cdn: O.Option<string>) =>
  (objects: FileInfo[]): AppErrorOr<S3ObjectInfo[]> => {
      ...
      TE.chain((res) => {
        return pipe(
          ...
          TE.chain((contents) => {
            ...
            const objects = A.array.traverse(TE.taskEither)(
              sortedContents,
              (k) =>
                pipe(
                  TE.fromEither(
...
export const uploadImage =
  (s3: S3, bucket: string) =>
  (key: string, image: Blob): AppErrorOr<void> =>
    pipe(
            ...
            A.filterMap(
              (x) =>
                sequenceS(O.Applicative)({
                  key: x.Key ? O.some(x.Key) : O.none,
                  lastModified: x.LastModified
                    ? O.some(x.LastModified.getTime())
                    : O.none,
                  state: O.some('ADDED'),
